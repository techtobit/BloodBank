...
const ServiceDetials: React.FC = () => {
	...
	useEffect(() => {
		const fetchServices = async () => {
			try {
				const response = await axios.get(`http://127.0.0.1:8000/services/list/${id}`)
	...
	useEffect(() => {
		const fetchReview = async () => {
			try {
				const response = await axios.get(`http://127.0.0.1:8000/services/review/?service=${id}`)
	...
	const handelSubmitReviw = (event:any)=>{
		...
		try {
			const response =axios.post(`http://127.0.0.1:8000/services/review/`, reviewData, {
				headers: {
					"Content-Type": "application/json"
				}
			})
