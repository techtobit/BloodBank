...
const Services: React.FC = () => {
	...
	useEffect(() => {
		const fetchServices = async () => {
			try {
				const response = await axios.get('http://127.0.0.1:8000/services/list/')
	...
	useEffect(() => {
		const fetchCategories = async () => {
			try {
				const response = await axios.get('http://127.0.0.1:8000/services/categories/')
	...
	const fetchFilteredCategory = async (categoryName: any, rating: any) => {
		try {
			const response = await axios.get(`http://127.0.0.1:8000/services/list/category/${categoryName}/`, {
				params:{
					avg_rating: rating,
				}
			})
					...
					<h2 className='font-bold pl-2 underline pb-2'>Filter by category</h2>
					{
						categorySlug?.map((slug, idx: number) => (
							<div key={idx} onChange={() => handelChangeCategory(slug?.category)} className="flex items-center ps-4 border border-gray-200 rounded dark:border-gray-700">
								...
								<label htmlFor="bordered-checkbox-2" className="w-full capitalize py-4 ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">	{slug?.category}</label>
							</div>
						))}
					...
					<h2 className='font-bold pl-2 underline py-2'>Filter by rateing</h2>
						{
							[5, 4, 3, 2, 1].map((rating, idx) => (
								...
								<label htmlFor="bordered-checkbox-2" className="w-full py-4 ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">  {ratings[rating]} ({rating})</label>
							</div>
							))
						}
